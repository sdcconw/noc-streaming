...
- 出力先URL
- Stream Key（任意）
- 入力種別（Browser / Test Pattern / SSH Terminal）
- 表示URLリスト（複数）
...
- 複数URL時: ジョブの `refresh_interval_sec`（切替間隔）ごとに次URLへ遷移
- 単一URL時: URLごとの `refresh_interval_sec`（URL更新間隔）ごとにリロード
- URLごとの `refresh_interval_sec=0` はリロード無効（切替のみ）
- 最小間隔は10秒
...
- SSH Terminal入力: Xvfb上でxtermを起動し、利用者がVNC/noVNCから任意の `ssh ...` を入力
- SSH Terminal入力時は背景色・文字色をジョブごとに設定可能
